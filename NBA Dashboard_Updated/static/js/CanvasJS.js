/**
* @preserve CanvasJS HTML5 & JavaScript Charts - v2.3.2 GA - https://canvasjs.com/ 
...
var updateCanvasSize = function (canvasList) {
	...
	if (this.canvas.width !== width * devicePixelBackingStoreRatio || this.canvas.height !== height * devicePixelBackingStoreRatio) {
		...
		for (var i = 0; i < canvasList.length; i++) {
			setCanvasSize(canvasList[i], width, height);
		}
...
var getAutoFontSize = function (standardSize, width, height) {
...
function createCanvas(width, height) {
	...
	if (isDebugMode) {
		this.canvasCount = 0 | this.canvasCount;
...
function setButtonState(chart, button, state) {
	...
	if (button.getAttribute("state") !== state) {
		...
		button.innerHTML = "<img style='height:95%; pointer-events: none;' src='" + base64Images[state].image + "' alt='" + chart._cultureInfo[state + "Text"] + "' />";
	}
}
...
CanvasJSObject.prototype.set = function (name, value, updateChart) {
	...
	if (updateChart) {
		var chart = this.stockChart || this.chart || this;
		chart.render();
	}
}
...
Title.prototype.setLayout = function () {
...
Subtitle.prototype.setLayout = Title.prototype.setLayout;
Subtitle.prototype.render = Title.prototype.render;

//#endregion Class SubTitle

//#region Class CanvasPool
function CanvasPool() {
	this.pool = [];
};

CanvasPool.prototype.get = function (width, height) {
...
CanvasJS.Chart = (function () {
	...
	function Chart(containerId, options) {
		...
		this.canvas.style.WebkitUserSelect = "none"; // Chrome, Safari, Opera
		this.canvas.style.MozUserSelect = "none"; // Firefox
		this.canvas.style.msUserSelect = "none"; // IE 10+
		this.canvas.style.userSelect = "none"; // Standard syntax
		...
		this.overlaidCanvas.style.WebkitUserSelect = "none"; // Chrome, Safari, Opera
		this.overlaidCanvas.style.MozUserSelect = "none"; // Firefox
		this.overlaidCanvas.style.msUserSelect = "none"; // IE 10+
		this.overlaidCanvas.style.userSelect = "none"; // Standard syntax
	...
	Chart.prototype.destroy = function () {
		...
		for (var i = 0; i < allDOMEventHandlers.length; i++) {
	...
	Chart.prototype._updateSize = function () {
		return updateCanvasSize.call(this, [this.canvas, this.overlaidCanvas, this._eventManager.ghostCanvas]);
	}
	...
	Chart.prototype._initialize = function () {
		...
		this.isNavigator = (!isNullOrUndefined(this.parent) && !isNullOrUndefined(this.parent._defaultsKey)) && this.parent._defaultsKey === "Navigator" ? true : false;
	...
	Chart.prototype.setLayout = function () {
		var plotAreaElements = this._plotAreaElements; //Elements to be rendered inside the plotArea
		...
		if (this.plotInfo.axisPlacement === "normal" || this.plotInfo.axisPlacement === "xySwapped") {
			...
			Axis.setLayout(this.axisX, this.axisX2, this.axisY, this.axisY2, this.plotInfo.axisPlacement, this.layoutManager.getFreeSpace());
		}
	}

	Chart.prototype.renderElements = function () {
		var plotAreaElements = this._plotAreaElements;

		if (this.title && !this.title.dockInsidePlotArea)
			this.title.render();


		for (var i = 0; i < this.subtitles.length; i++) {

			if (!this.subtitles[i].dockInsidePlotArea)
		...
		if (this.plotInfo.axisPlacement === "normal" || this.plotInfo.axisPlacement === "xySwapped") {
			Axis.render(this.axisX, this.axisX2, this.axisY, this.axisY2, this.plotInfo.axisPlacement);
		} else if (this.plotInfo.axisPlacement === "none") {
		...
		for (var index = 0; index < plotAreaElements.length; index++) {
			plotAreaElements[index].setLayout();
				...
				if (this._axes[i].dataInfo.dataPointYPositiveSums) {
					...
					for (j in dataPointYPositiveSums) {
						if (dataPointYPositiveSums.hasOwnProperty(j)) {
							...
							var index = Math.floor((ySum - plotAreaYMin) / yDivisor);
	...
	Chart.prototype.getAutoFontSize = getAutoFontSize;
	...
	Chart.prototype.clearCanvas = clearCanvas;
	...
	Chart.prototype._touchEventHandler = function (ev) {
		...
		try {
			...
			if ((Math.abs(dy) > 1 && !!this._lastTouchData.scroll) || (Math.abs(dy) > 5 && dt < 250)) {
		...
		for (var i = 0; i < mouseEvents.length; i++) {
			...
			if ((!isNullOrUndefined(this._lastTouchData.scroll) && !this._lastTouchData.scroll) || (!this._lastTouchData.scroll && dt > 250) || type === "click") {
	...
	Chart.prototype._zoomPanToSelectedRegion = function (px1, py1, px2, py2, keepAxisIndependent) {
		...
		if (validateRegion.isValid) {
			for (var i = 0; i < axesWithValidRange.length; i++) {
				...
				if (this.syncCharts) {
					this.syncCharts(param.val1, param.val2)
				}
			}
		}
	...
	Chart.prototype.renderFunnel = function (plotUnit) {
		...
		function dataToAreaRatio() {
			...
			var headArea = (y3 - y1) * ((x2 - x1) + (x4 - x3)) / 2;
			...
			for (var i = 0; i < dataPoints.length; i++) {
				...
				if (!neckRegion) {
					var sqrt = m1 * m1 * width1 * width1 - (4 * Math.abs(m1) * areaForThisDataPoint);
					if (sqrt < 0) {
						width2 = lengthOfNeck;
		...
		function repositionLabels() {
			...
			var previousLabel, previousTextBlock, currentLabel, currentTextBlock, nextLabel, nextTextBlock;
		...
		function explodeToggle(_this, selectedSection, animDuration) {
			...
			var animatingObject = {
				...
				changeSection: function (fractionComplete) {
					...
					var direction, currentSection, newY;
	...
	Legend.prototype.setLayout = function () {
		...
		for (var i = 0; i < this.dataSeries.length; i++) {
			...
			if (dataSeries.type !== "pie" && dataSeries.type !== "doughnut" && dataSeries.type !== "funnel" && dataSeries.type !== "pyramid") {
				...
				if (dataSeries.type === "error") this.errorMarkerColor.push(markerColor);
	...
	Legend.prototype.render = function () {
		var left = this.bounds.x1;

		var top = this.bounds.y1;
		var markerMargin = this.markerMargin;
		...
		for (var i = 0; i < rows.length; i++) {
			...
			for (var itemIndex = 0; itemIndex < row.items.length; itemIndex++) {
				...
				if (item.chartType === "error") {
					...
					var markerBorderThickness = markerSize / 8;
	...
	Axis.prototype.createLabelsAndCalculateWidth = function () {
		...
		var padding = this.chart.isNavigator ? 0 : 5;
		if (this._position === "left" || this._position === "right") {
		...
		var axisWidth = (this.labelPlacement === "inside") ? axisWidth = titleHeight + padding : titleHeight + maxLabelEffectiveWidth + this.tickLength + padding;
	...
	Axis.prototype.createLabelsAndCalculateHeight = function () {
		...
		var axisHeight = (this.labelPlacement === "inside") ? axisHeight = titleHeight + padding : titleHeight + maxLabelEffectiveHeight + this.tickLength + padding;

		return axisHeight;
	}

	//Static Method that co-ordinates between primary & secondary axisX, axisY and calculates all necessary axis parameters
	Axis.setLayout = function (axisX, axisX2, axisY, axisY2, axisPlacement, freeSpace) {
		var x1, y1, x2, y2;
		var chart = axisX[0] ? axisX[0].chart : axisX2[0].chart;
		var maxPadding = chart.isNavigator ? 0 : 10;
		var ctx = chart.ctx;
			...
			while (i < 4) {
				...
				if (axisY && axisY.length > 0) {
					...
					for (var k = 0; k < axisY.length; k++) {
						...
						axisYMargin += axisY[k] && !chart.isNavigator ? axisY[k].margin : 0;
					}
				...
				x2 = Math.round(freeSpace.x2 - axisY2TotalWidth - axisY2Margin > chart.width - maxPadding ? chart.width - maxPadding : freeSpace.x2 - axisY2TotalWidth - axisY2Margin);
				...
				if (axisX && axisX.length > 0) {
					...
					for (var k = 0; k < axisX.length; k++) {
						...
						axisXMargin += axisX[k] && !chart.isNavigator ? axisX[k].margin : 0;
					}
				...
				if (axisX2 && axisX2.length > 0) {
					...
					for (var k = 0; k < axisX2.length; k++) {
						...
						axisX2Margin += axisX2[k] && !chart.isNavigator ? axisX2[k].margin : 0;
					}
				...
				if (axisX && axisX.length > 0) {
					for (var k = 0; k < axisX.length; k++) {
						if (axisX[k]) {
							...
							if (axisX[k] && axisX[k].labelAutoFit && !isNullOrUndefined(firstLabelPosition) && !isNullOrUndefined(lastLabelPosition) && !chart.isNavigator) {
				...
				if (axisX2 && axisX2.length > 0) {
					for (var k = 0; k < axisX2.length; k++) {
						...
						axisX2[k].lineCoordinates.x2 = Math.round(freeSpace.x2 - axisY2TotalWidth - axisY2Margin > chart.width - maxPadding ? chart.width - maxPadding : freeSpace.x2 - axisY2TotalWidth - axisY2Margin);
				...
				if (axisY && axisY.length > 0) {
					for (var k = 0; k < axisY.length; k++) {
						var padding = chart.isNavigator ? 0 : 10;
						...
						if (axisY[k]) {
							...
							padding = axisY[k]._labels && axisY[k]._labels.length > 0 ? axisY[k]._labels[axisY[k]._labels.length - 1].textBlock.height / 2 : maxPadding;
							y1 = Math.round(freeSpace.y1 + axisX2TotalHeight + axisX2Margin < Math.max(padding, maxPadding) ? Math.max(padding, maxPadding) : freeSpace.y1 + axisX2TotalHeight + axisX2Margin);
							...
							if (!(axisX.length > 0)) {
								padding = axisY[k]._labels && axisY[k]._labels.length > 0 ? axisY[k]._labels[0].textBlock.height / 2 : maxPadding;
							} else {
								padding = 0;
							}
	...
	//Static Method that renders all axes
	Axis.render = function (axisX, axisX2, axisY, axisY2, axisPlacement) {
		var x1, y1, x2, y2;
		var chart = axisX[0] ? axisX[0].chart : axisX2[0].chart;
		var ctx = chart.ctx;
		var axes = chart._axes;
		...
		if (chart.alignVerticalAxes) {
			chart.alignVerticalAxes();
		}
	...
	Axis.prototype.calculateLogarithmicAxisParameters = function () {
		...
		var linearRange = (isNaN(this.viewportMaximum) || this.viewportMaximum === null ? max : this.viewportMaximum) - (isNaN(this.viewportMinimum) || this.viewportMinimum === null ? min : this.viewportMinimum);
	...
	ToolTip.prototype._updateToolTip = function (mouseX, mouseY, closest) {
		...
		if (this.shared && this.enabled && this.chart.plotInfo.axisPlacement !== "none") {
			...
			if (this.currentSeriesIndex >= 0) {
				...
				if (!isNullOrUndefined(entry) && !isNullOrUndefined(entry.dataPoint) && !isNullOrUndefined(entry.dataPoint.y)) {
...
CanvasJS.Chart.version = "v2.3.2 GA";
